...
from .Boschloo import Boschloo
import os
from pkg_resources import resource_filename

boschloo: Boschloo = None
...
def main(**kwargs):
    ...
    if len(kwargs) == 0:
        args, cutoffs = parse_scoary_args()
    else:
        args = kwargs["args"]
    ...
    # If the test argument was used, all settings are overridden and
    # defaulted
    if args.test:
        args.boschloo = False
    ...
    try:
        ...
        if ((args.newicktree is not None) and
            ...
            sys.exit("Could not find the custom tree file: %s"
                     % args.newicktree)
        if not all([(1.0 >= p > 0.0) for p in args.p_value_cutoff]):
        ...
        with open(args.genes, "rU") as genes, \
            ...
            if args.newicktree is None and not args.no_pairwise:
                log.info("Creating Hamming distance matrix based on gene "
                         "presence/absence")
                TDM = create_triangular_distance_matrix(zeroonesmatrix,
                                                        strains)
                QT = populate_quad_tree_with_distances(TDM)
                ...
                from .nwkhandler import read_tree_from_file
                upgmatree, members = read_tree_from_file(args.newicktree)
                    ...
                    if args.restrict_to is None:
                        ...
                        if all(i in members for i in strains):
                            ...
                            prune = [i for i in members if
                                     i not in strains]
                            upgmatree = prune_for_missing(upgmatree, prune)
                        else:
        ...
        filtopts = filtration_options(cutoffs, args.collapse)
        for line in filtopts:
            log.info(line)

        log.info("Tallying genes and performing statistical analyses")

        RES_and_GTC = setup_results(genedic, traitsdic, args.collapse, args.boschloo)
        RES = RES_and_GTC["Results"]
        GTC = RES_and_GTC["Gene_trait_combinations"]

        if args.upgma_tree:
            store_tree_to_file(upgmatree,
                               args.outdir,
                               time=currenttime)
...
###############################
def roary_csv_to_dic(genefile, delimiter, grabcols, startcol=14,
    ...
    if writereducedset:
        file = open(reduce_set(genefile, delimiter, grabcols, startcol,
...
def reduce_set(genefile, delimiter, grabcols, startcol=14,
...
def csv_to_dic(csvfile, delimiter, allowed_isolates, strains) -> (dict, dict):
    """
...
################################
def create_triangular_distance_matrix(zeroonesmatrix, strainnames):
    """
...
def populate_quad_tree_with_distances(TDM):
    """
...
def prune_for_missing(tree, Prunedic):
    """
    ...
    if isinstance(tree[0], list):
        tree[0] = prune_for_missing(tree[0], Prunedic)
    # Right node is a subtree, go deeper
    if isinstance(tree[1], list):
        tree[1] = prune_for_missing(tree[1], Prunedic)
...
def store_tree_to_file(upgmatree, outdir, time=""):
    """
...
#####################################
def setup_results(genedic, traitsdic, collapse, boschloo: bool):
    """
    ...
    test_function = boschloo_test if boschloo else fisher_test
    ...
    for trait in traitsdic:
        ...
        for gene in genedic:
            ...
            # for boschloo, column sums must be fixed, i.e. obs_table[0][0] + obs_table[1][0] == n_positive_trait
            obs_table = [
                [
                    stat_table["tpgp"],  # positive trait
                    stat_table["tngp"],  # negative trait
                ],
                [
                    stat_table["tpgn"],  # positive trait
                    stat_table["tngn"],  # negative trait
                ]
            ]
            ...
            if obs_tuple in fisher_calculated_values:
                ...
                res = test_function(obs_table)
                p_value = res.p_value
                odds_ratio = res.odds_ratio
                fisher_calculated_values[obs_tuple] = res.as_tuple()
...
#################################
def store_results(results, max_hits, cutoffs, upgmatree, GTC, Prunedic,
    ...
    for trait in results:
        ...
        store_trait_results(results[trait], trait, max_hits, cutoffs,
                            upgmatree, GTC, Prunedic, outdir, permutations,
                            num_threads, no_pairwise, genedic,
                            extracolstoprint, firstcolnames, time, delimiter)


def store_trait_results(trait, traitname, max_hits, cutoffs, upgmatree,
    ...
    with open(fname, "w") as outfile:
        ...
        sort_instructions = sort_results_and_set_key(trait)
        if max_hits is None:
            max_hits = len(trait)
        ...
        if not no_pairwise:
            ...
            if len(Prunedic[traitname]) > 0:
                upgmatree = prune_for_missing(upgmatree, Prunedic[traitname])
            ...
            if multithreaded:
                ...
                # all subprocs
                Progress = Value("h", 0)
                pool = Pool(processes=num_threads, initializer=init_process,
                            initargs=(Progress,))
                ...
                Threadresults = pool.imap(pairwise_comparison, all_args)
                ...
                Threadresults = [pairwise_comparison(all_args)]
            sys.stdout.write("\r100.00%")
            sys.stdout.flush()
            sys.stdout.write("\n")
            ...
            if ("I" in cutoffs) or ("B" in cutoffs) or ("BH" in cutoffs):
                sort_instructions = sort_results_and_set_key(Filteredresults)
            elif ("PW" in cutoffs) or ("EPW" in cutoffs):
                sort_instructions = \
                    sort_results_and_set_key_pairwise(Filteredresults, cutoffs)
            elif ("P" in cutoffs):
                sort_instructions = \
                    sort_results_and_set_key(Filteredresults, key="Empirical_p")
            else:
...
def pairwise_comparison(nestedlist):
    """
    ...
    for genenumber in domain:
        ...
        Max_pairwise_comparisons = convert_upgma_to_phylotree(
        ...
        if decide_if_break(cutoffs, resultscontainer[currentgene]):
...
def permute(tree, GTC, permutations, cutoffs):
    ...
    # the unpermuted
    r = 0
    Unpermuted_tree = convert_upgma_to_phylotree(tree, GTCcopy)
    ...
    if proceed:
        for i in range(permutations):
            ...
            NewPhyloTree = convert_upgma_to_phylotree(tree, PermutedGTC)
...
##############################
def find_closest_index(sortedlist, value, index=None):
    """
    ...
    if value == middlevalue:
        ...
        index += find_closest_index(sortedlist[:middleindex],
                                    value,
                                    index)
    else:
        # Index in right part of list
        index += (middleindex +
                  find_closest_index(sortedlist[middleindex:],
                                     value,
                                     index))
...
class ContingencyResult:
    def __init__(self, p_value: float, odds_ratio: Optional[float] = None, statistic: Optional[float] = None):
        self.p_value = p_value
        self.odds_ratio = odds_ratio
        self.statistic = statistic

    def as_tuple(self) -> (float, float):
        return self.odds_ratio, self.p_value


def fisher_test(table: [[int, int], [int, int]]) -> ContingencyResult:
    fisher = ss.fisher_exact(table=table)
    return ContingencyResult(p_value=fisher[1], odds_ratio=fisher[0])


def boschloo_test(table: [[int, int], [int, int]]) -> ContingencyResult:
    pvalue, statistic = boschloo.get_or_create(*[j for i in table for j in i])
    return ContingencyResult(p_value=pvalue, statistic=statistic)


def sort_results_and_set_key(genedic, key="p_v"):
    """
...
def sort_results_and_set_key_pairwise(genedic, correctionmethod):
    """
    A method for returning a dictionary where genes are sorted by 
...
def filtration_options(cutoffs, collapse):
    """
...
def decide_if_break(cutoffs, currentgene):
    """
